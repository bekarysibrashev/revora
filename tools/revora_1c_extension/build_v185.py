"""Build and verify Revora 1C extension v18.5 from the v18.4 archive.

v18.5 exists because v18.4's automatic verdict was wrong.

v18.4 concluded "more than two numeric columns, therefore the wrong one is
taken". The first real run returned an ИТОГО row of
216 | 4 104 748,35 | 675 986,65 | 4 780 735,00, where the second and third
numbers sum to the fourth exactly. That is "count | part | part | sum of
parts", and the number being taken may well be the cost after all -- the
verdict fired on a weak signal and jumped to a conclusion the data does not
support.

So the verdict is now derived from evidence rather than shape:

- Column headers are captured, so what each number IS stops being a guess.
- The direct query reports its own row count, quantity and
  before-discount total, and whether its text uses ВЫБРАТЬ РАЗРЕШЕННЫЕ.
- The quantity comparison decides between the two remaining causes. If the
  query's quantity matches the report's, both see the same rows and the
  disagreement is about which field is summed. If it does not, the query
  is losing rows -- to РАЗРЕШЕННЫЕ under the current user's RLS (the DCS
  runs without the privileged-mode restriction and can see more), or to a
  settings-level filter absent from the dataset query text.
...
SRC_ZIP = BASE_DIR / "RevoraExtensionReady-v18.4-complete-sync.zip"
OUT_ZIP = BASE_DIR / "RevoraExtensionReady-v18.5-complete-sync.zip"
