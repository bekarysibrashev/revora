"""Build and verify Revora 1C extension v18.4 from the v18.3 archive.

v18.4 adds nothing to what is measured. v18.3 already sent the ИТОГО row
of the standard report with every service_revenue snapshot, but only into
the snapshot -- so the evidence was visible in Revora and nowhere else,
while the person who runs a sync is looking at the 1C sync window.

This revision carries the same fields into ПредставлениеИтоговСинхронизации
and prints them there, together with a plain-language verdict:

- more than two numeric columns in the ИТОГО row means the "first is a
  quantity, second is the cost" assumption is wrong for this
  configuration, and the discrepancy is a column-choice bug;
- exactly two, with a non-zero difference, means the scrape reads the
  right number and the standard report and the Продажи register genuinely
  disagree -- a document-level question (refunds, reversals, cancelled
  documents, DCS filters, rows with no employee).

Every number of the ИТОГО row is printed alongside the verdict so it can
...
SRC_ZIP = BASE_DIR / "RevoraExtensionReady-v18.3-complete-sync.zip"
OUT_ZIP = BASE_DIR / "RevoraExtensionReady-v18.4-complete-sync.zip"
