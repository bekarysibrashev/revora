...
- LLM AI-аналитик с role/branch scope, проверяемыми источниками и защитой от выдуманных чисел.

Telegram-рассылка остаётся следующим этапом. Диалоговый AI-аналитик реализован отдельным разделом `/analyst`; для реальных ответов требуется backend-переменная `OPENAI_API_KEY`.
