...
type Connection = { id:string; name:string; provider:string; status:string; settings?:{last_synced_at?:string;last_entity?:string;[key:string]:unknown} };
...
type TelegramRuntimeStatus = { configured:boolean;running:boolean;mode:string;last_error:string|null };
...
function OneCIntegration(){
  ...
  const lastSync=currentConnection?.settings?.last_synced_at;
  const syncAgeMinutes=lastSync?(Date.now()-new Date(lastSync).getTime())/60000:null;
  const syncHealthy=currentConnection?.status==="connected"&&syncAgeMinutes!==null&&syncAgeMinutes<=90;
  ...
  return <section className="panel">
    ...
    {!oneCConnections.length?<button className="primary small" onClick={()=>create.mutate()} disabled={create.isPending}>{create.isPending?<><span className="spinner" aria-hidden="true"/>Создаём…</>:"Создать подключение 1С"}</button>:<>
      ...
      {currentConnection&&<div className={syncHealthy?"success-box":"error-box"}><strong>{syncHealthy?"1С синхронизируется автоматически":"Синхронизация 1С требует проверки"}</strong><br/>{lastSync?`Последний успешно принятый пакет: ${new Date(lastSync).toLocaleString("ru-RU")}`:"Revora ещё не получила операционный пакет 1С."}{syncAgeMinutes!==null&&syncAgeMinutes>90?<><br/>Последние данные старше 90 минут — проверьте регламентное задание в 1С.</>:null}</div>}
...
function TelegramStaff(){
  const qc=useQueryClient();
  const runtime=useQuery({queryKey:["telegram-runtime"],queryFn:()=>api<TelegramRuntimeStatus>("/telegram/status"),refetchInterval:30000});
  ...
  return <div className="admin-stack">
    <section className="panel"><h2>Состояние Telegram-бота</h2>{runtime.data?<div className={runtime.data.running?"success-box":"error-box"}><strong>{runtime.data.running?"Бот работает постоянно":"Бот не запущен"}</strong><br/>{runtime.data.running?"Задания и ответы принимаются внутри сервера Revora — отдельная платная служба не нужна.":runtime.data.configured?"Сервер пытался запустить бота. Проверьте ошибку ниже.":"Добавьте TELEGRAM_BOT_TOKEN в переменные окружения API Revora."}{runtime.data.last_error?<><br/>{runtime.data.last_error}</>:null}</div>:<p className="muted">Проверяем состояние бота…</p>}</section>
