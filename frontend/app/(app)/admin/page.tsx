...
type TelegramEmployee = { id:string;branch_id:string|null;role:string;telegram_user_id:number;username:string|null;full_name:string;is_active:boolean;registered_at:string;last_seen_at:string };
type TelegramInvitation = { id:string;code:string;code_hint:string;role:string;branch_id:string|null;expires_at:string;max_uses:number };
...
function TelegramStaff(){
  ...
  const[role,setRole]=useState("administrator"),[branch,setBranch]=useState(""),[invite,setInvite]=useState<TelegramInvitation|null>(null);
  ...
  async function createInvite(e:FormEvent){e.preventDefault();setError("");setCopied(false);try{setInvite(await api<TelegramInvitation>("/telegram/invitations",{method:"POST",body:JSON.stringify({role,branch_id:branch||null,expires_in_hours:24,max_uses:1})}))}catch(e){setError(e instanceof Error?e.message:"Не удалось создать приглашение")}}
  ...
  return <div className="admin-stack">
    <section className="panel"><h2>Приглашение в Telegram-бота</h2><p className="muted">Код одноразовый, действует 24 часа и сразу закрепляет роль сотрудника. Передавайте его лично.</p><form className="form-grid user-form" onSubmit={createInvite}><label>Роль<select value={role} onChange={e=>setRole(e.target.value)}><option value="manager">Руководитель</option><option value="administrator">Администратор</option><option value="sales_manager">Менеджер продаж</option></select></label><label>Филиал<select value={branch} onChange={e=>setBranch(e.target.value)} required={branchRequired}><option value="">Все / не назначен</option>{branches.data?.items.filter(x=>x.is_active).map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label><button className="primary small">Создать код</button></form>{invite&&<div className="success-box"><strong>Код показан только сейчас</strong><br/><code>{invite.code}</code> <button type="button" className="small" onClick={copyInvite}>{copied?"Скопировано":"Копировать"}</button><br/><small>Истекает: {new Date(invite.expires_at).toLocaleString("ru-RU")}</small></div>}</section>
    <section className="panel"><h2>Сотрудники в боте</h2><div className="table-wrap"><table><thead><tr><th>Сотрудник</th><th>Роль</th><th>Telegram</th><th>Статус</th><th></th></tr></thead><tbody>{employees.data?.items.map(x=><tr key={x.id}><td><strong>{x.full_name}</strong></td><td>{roleLabels[x.role]}</td><td>{x.username?`@${x.username}`:`ID ${x.telegram_user_id}`}</td><td><span className={x.is_active?"badge active":"badge"}>{x.is_active?"Активен":"Отключён"}</span></td><td><button className={x.is_active?"danger small":"small"} onClick={()=>toggle(x)}>{x.is_active?"Отключить":"Включить"}</button></td></tr>)}</tbody></table></div></section>
