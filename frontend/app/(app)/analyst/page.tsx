...
export default function AnalystPage(){
  ...
  return <><PageHeader title="Data Science Lab" subtitle="Задавайте вопросы по 1С, продажам, финансам, врачам и маркетингу — ответы строятся только по проверяемым данным Revora" action={<button className="primary" onClick={()=>create.mutate()} disabled={create.isPending}>Новый анализ</button>}/>
    <div className="analyst-layout"><aside className="analyst-sessions"><div className="analyst-side-title"><strong>История</strong><span>{sessions.data?.items.length||0}</span></div>{sessions.isLoading&&<p>Загрузка…</p>}{sessions.data?.items.map(item=><button key={item.id} className={active===item.id?"active":""} onClick={()=>setActive(item.id)}><strong>{item.title}</strong><small>{new Date(item.last_message_at||item.created_at).toLocaleDateString("ru-RU")}</small></button>)}{!sessions.isLoading&&!sessions.data?.items.length&&<p>Диалогов пока нет</p>}</aside>
      <section className="analyst-chat"><header><div className="analyst-mark">AI</div><div><strong>Revora AI</strong><small><i/>Только агрегированные данные</small></div></header><div className="analyst-messages">
        ...
        {active&&messages.isLoading&&<div className="center-state">Загружаем диалог…</div>}
